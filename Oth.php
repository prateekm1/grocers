	...
	if($_GET["action"]=="delete")
	{
		foreach($_SESSION["shopping_cart"] as $keys =>$values)
		{
			if($values["Item_Id"]==$_GET["Id"])
			{
				...
				echo '<script>window.location="Oth.php"</script>';
			}
		}
	}
}
...
<link href="css/bootstrap.css" rel="stylesheet" type="text/css" media="all" />
	...
	if(mysqli_num_rows($result)>0)
	{
		while($row=mysqli_fetch_array($result))
		{
			...
			?><center>
			<div class="col-md-4">
			<form method="post" action="Oth.php?action=add&Id=<?php echo $row["Id"];?>">
	...
	if(!empty($_SESSION["shopping_cart"]))
	{
	 ...
	 ?>
		...
		<td><a href="Oth.php?action=delete&Id=<?php echo $values["Item_Id"];?>"><span class="text-danger">Remove</span></a></td>
		</tr>
	<?php 
